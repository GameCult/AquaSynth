...
public static class CultCachePatchDocumentCatalog
{
    ...
    [
        ...
        new("patches/advanced/subtractive-drums.aqua", "advanced", "subtractive-drums", "BuiltInScripts.ReferenceScripts", "Filtered subtractive drum voices with pitched bodies, noise skins, and bright metal."),
        new("patches/advanced/syrinx-voice.aqua", "advanced", "syrinx-voice", "BuiltInScripts.ReferenceScripts", "Graph-native paired-labium voice with modulated pressure, opening, and beak radiation."),
    ...
    [
        ...
        PatchClaim("pink-trombone/current-aquasynth-graph", null, "Pink Trombone modular waveguide", "src/AquaSynth.Core/PinkTromboneReference.cs", "MIT source-readable parity pressure", "AquaSynth expresses the PT pressure lane as typed acoustic graph records rather than a source-filter proxy.", "Speech anatomy pressure for the graph-native vocal tract.", "pressure", "PinkTromboneLogMelParityTests graph artifacts", MissingReceipt("Pink Trombone reference renderer versus AquaSynth typed acoustic graph rebuild", "typed acoustic graph lowering, log-mel reports, and user listening corrections", "No accepted speech witness yet; closure/contact state and source impedance remain contaminated.", "The old proxy is dead. The graph still has to earn its aria."), "structural graph lowering plus non-passing log-mel pressure", ["not intelligible speech parity yet", "closure/contact pressure storage remains incomplete", "glottal source is bounded but not exact PT LF coefficients"]),
