...
public sealed class AquaDaemonClient : IAsyncDisposable
{
    ...
    public async Task<string> RenderNoteAsync(DingInstrument instrument, float frequency, float gain, CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            var envelope = new
            {
                command = "instrument.sample",
                payload = new
                {
                    commandId = id,
                    patchId = $"dings.{instrument.Id}",
                    faustName = $"dings_{instrument.Id.Replace('-', '_')}",
                    script = instrument.Script,
                    durationSeconds = 1.5f,
                    gain,
                    controls = new Dictionary<string, float> { ["/ding/frequency"] = frequency },
                    revision = 1
                }
            };
            ...
            var line = await process.StandardOutput.ReadLineAsync(cancellationToken)
                ?? throw new InvalidOperationException("Aqua daemon closed before returning a render receipt.");
    ...
    private void EnsureStarted()
    {
        ...
        var daemonDll = ResolveDaemonDll(root);
        var arguments = $"\"{daemonDll}\" daemon --store \"{Path.Combine(root, ".aquasynth", "dings-mcp")}\"";
        process = Process.Start(new ProcessStartInfo("dotnet", arguments)
        {
    ...
    private static string ResolveDaemonDll(string root)
    {
        var configured = Environment.GetEnvironmentVariable("AQUASYNTH_DAEMON_DLL");
        if (!string.IsNullOrWhiteSpace(configured) && File.Exists(configured)) return Path.GetFullPath(configured);
        var bin = Path.Combine(root, "tools", "AquaSynthDaemon", "bin");
        var candidate = Directory.EnumerateFiles(bin, "AquaSynthDaemon.dll", SearchOption.AllDirectories)
            .OrderByDescending(File.GetLastWriteTimeUtc)
            .FirstOrDefault();
        return candidate ?? throw new InvalidOperationException("AquaSynthDaemon.dll is not built. Build AquaSynthDingsMcp once or set AQUASYNTH_DAEMON_DLL.");
    }
    ...
    private static string FindRepositoryRoot()
    {
        ...
        for (var dir = new DirectoryInfo(Environment.CurrentDirectory); dir is not null; dir = dir.Parent)
            if (File.Exists(Path.Combine(dir.FullName, "AquaSynth.sln"))) return dir.FullName;
        throw new InvalidOperationException("Cannot locate AquaSynth.sln. Start the MCP service from the AquaSynth repository or set AQUASYNTH_DAEMON_DLL.");
    }
